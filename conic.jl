...
function addTower(threeDimImplementation::Function, epsilon::Float64=0.01)
    ...
    function implementTower(model::Model, y, ys, t, threeDimCones::Vector{Vector{JuMP.Variable}})
         ...
         dim = length(y)
         K   = iceil(log(2,dim))
         skb = iceil(log(4,(16/9)*pi^(-2.0)*log(1+epsilon)))
...
function getCorrectedSolution(prob::MISOCPInput, z_lower, z_upper, hz, hx, hy, hys,
...
function liftedlpsolve(prob::MISOCPInput, masterimplementation, correctionimplementation,
    ...
    hmodel, hx, hz, hy, hys, ht, hthreeDimCones =
        buildModel(prob, correctionimplementation, correctionsolver, equality; relaxed=true)
    ...
    function branchcallback(cb)
        ...
        if feasible
            ...
            if !fixed
                obj = getCorrectedSolution(prob, [ lb[z[i].col]  for i=1:length(prob.f) ],
    ...
    function inccallback(cbdata)
        ...
        if reject
            obj = getCorrectedSolution(prob, getValue(z), getValue(z), hz, hx, hy, hys, ht,
                                       hmodel, hthreeDimCones, xstar, zstar, ystar, ysstar,
                                       tstar, threeDimConesstar, incumbentobj)
            if obj < incumbentobj
                incumbentobj = obj
                newincumbent = true
...
function lazycutsolve(prob::MISOCPInput, masterimplementation, correctionimplementation,
    ...
    function towerSeparator(cb)
        ...
        if separated && dot(prob.c,getValue(x)) + dot(prob.f,getValue(z)) < incumbentobj
            ...
            obj = getCorrectedSolution(prob, z_val,z_val, hz, hx, hy, hys, ht, hmodel, hthreeDimCones,
                                       xstar, zstar, ystar, ysstar, tstar, threeDimConesstar, incumbentobj)
